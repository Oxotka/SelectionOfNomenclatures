...
Процедура ПодборТоваров_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоРасширеннаяФорма Тогда
		...
		ЕстьПравоДоступаКОтчету = ПравоДоступа("Использование", Метаданные.Отчеты.ОстаткиИДоступностьТоваров);
...
Процедура ПолучитьИнформациюОТовареПриПродаже(Форма) Экспорт
	...
	Если Не Форма.ОтображатьОстатки Или
		Форма.ПодборВариантовОбеспечения
...
Функция ЦенаПродажиИОстаткиТовара(Номенклатура, Характеристика, Соглашение, Валюта, Склады, ВидыЦен) Экспорт
	...
	Если ЗначениеЗаполнено(Характеристика) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "{(Номенклатура).* КАК Номенклатура}", "И Номенклатура = &Номенклатура И Характеристика = &Характеристика");
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "{(Номенклатура).* КАК Номенклатура}", "И Номенклатура = &Номенклатура");
...
Процедура ИзменитьТекстЗапроса()
	...
	Если СтрНайти(СписокНоменклатура.ТекстЗапроса, "ВтОжидаемоеПоступление") > 0 Тогда 
		Возврат;
	КонецЕсли;
	...
	|ВЫБРАТЬ
	|	ОжидаемоеПоступление.Номенклатура КАК Номенклатура,
	|	СУММА(ОжидаемоеПоступление.Запас) КАК Ожидается
	|ПОМЕСТИТЬ ВтОжидаемоеПоступление
	|ИЗ
	|	РегистрСведений.РаспределениеЗапасов КАК ОжидаемоеПоступление
	|ГДЕ
	|	ОжидаемоеПоступление.Склад В ИЕРАРХИИ (&Склад)
	|	И ОжидаемоеПоступление.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОжидаемоеПоступление)
	|СГРУППИРОВАТЬ ПО
	|	ОжидаемоеПоступление.Номенклатура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|" + СписокНоменклатура.ТекстЗапроса;
	...
	Если ТипДокумента = Тип("ДокументСсылка.ПеремещениеТоваров")
		...
		СписокНоменклатура.ТекстЗапроса = СтрЗаменить(СписокНоменклатура.ТекстЗапроса, "СправочникНоменклатура.Артикул КАК Артикул,", "СправочникНоменклатура.Артикул КАК Артикул,
			|	ЕСТЬNULL(ОжидаемоеПоступление.Ожидается, 0) КАК Ожидается,");
		СписокНоменклатура.ТекстЗапроса = СтрЗаменить(СписокНоменклатура.ТекстЗапроса, "Справочник.Номенклатура КАК СправочникНоменклатура", "Справочник.Номенклатура КАК СправочникНоменклатура
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВтОжидаемоеПоступление КАК ОжидаемоеПоступление
			|		ПО СправочникНоменклатура.Ссылка = ОжидаемоеПоступление.Номенклатура");
		СписокНоменклатура.ТекстЗапроса = СтрЗаменить(СписокНоменклатура.ТекстЗапроса, "0 КАК Цена,", "ВЫРАЗИТЬ(ЕСТЬNULL(ЦеныНоменклатуры1.Цена, 0) * ВЫБОР
			|			КОГДА &Валюта <> ЦеныНоменклатуры1.Валюта
			|				ТОГДА ВЫБОР
			|						КОГДА ЕСТЬNULL(КурсыСрезПоследнихВалютаЦены1.Кратность, 0) > 0
			|								И ЕСТЬNULL(КурсыСрезПоследнихВалютаЦены1.Курс, 0) > 0
			|								И ЕСТЬNULL(КурсыСрезПоследнихВалютаДокумента1.Кратность, 0) > 0
			|								И ЕСТЬNULL(КурсыСрезПоследнихВалютаДокумента1.Курс, 0) > 0
			|							ТОГДА КурсыСрезПоследнихВалютаЦены1.Курс * КурсыСрезПоследнихВалютаДокумента1.Кратность / (КурсыСрезПоследнихВалютаДокумента1.Курс * КурсыСрезПоследнихВалютаЦены1.Кратность)
			|						ИНАЧЕ 0
			|					КОНЕЦ
			|			ИНАЧЕ 1
		...
		СписокНоменклатура.ТекстЗапроса = СтрЗаменить(СписокНоменклатура.ТекстЗапроса, "Справочник.Номенклатура КАК СправочникНоменклатура", "Справочник.Номенклатура КАК СправочникНоменклатура
			...
			|				,
			|				ВидЦены = &ВидыЦен
			|					И ХарактеристикаЦО = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка) {(Номенклатура).* КАК Номенклатура}) КАК ЦеныНоменклатуры1
			|		ПО (ЦеныНоменклатуры1.Номенклатура = СправочникНоменклатура.Ссылка)
			|			И (ЦеныНоменклатуры1.ХарактеристикаЦО = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка))
			|			И (ЦеныНоменклатуры1.ВидЦены = &ВидыЦен)
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&Дата, ) КАК КурсыСрезПоследнихВалютаЦены1
			|		ПО (КурсыСрезПоследнихВалютаЦены1.Валюта = ЦеныНоменклатуры1.Валюта)
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&Дата, Валюта = &Валюта) КАК КурсыСрезПоследнихВалютаДокумента1
			|		ПО (ИСТИНА)");
		...
		СписокНоменклатура.ТекстЗапроса = СтрЗаменить(СписокНоменклатура.ТекстЗапроса, "Справочник.Номенклатура КАК СправочникНоменклатура", "Справочник.Номенклатура КАК СправочникНоменклатура
			...
			|				,
			|				ВидЦены = &ВидыЦен
			|					И Характеристика = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка) {(Номенклатура).* КАК Номенклатура}) КАК ЦеныНоменклатуры1
			|		ПО (ЦеныНоменклатуры1.Номенклатура = СправочникНоменклатура.Ссылка)
			|			И (ЦеныНоменклатуры1.Характеристика = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка))
			|			И (ЦеныНоменклатуры1.ВидЦены = &ВидыЦен)
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&Дата, ) КАК КурсыСрезПоследнихВалютаЦены1
			|		ПО (КурсыСрезПоследнихВалютаЦены1.Валюта = ЦеныНоменклатуры1.Валюта)
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&Дата, Валюта = &Валюта) КАК КурсыСрезПоследнихВалютаДокумента1
